...
import aux as A
import os
os.makedirs(A.OUTPUT_IMG/'arma_garch', exist_ok=True,)
...
class arma_garch():
    ...
    #Finds parameters p, d, q for ARIMA(p, d, q) which maximizes 
    # MLE-BIC for the data serie X.
    ...
    def plotAnomalyDetection(self, D, D_truth, D_unpolluted, D_guesses, D_means, D_vars):
        for row in range(min(len(D), 10)):
            ...
            plt.savefig(A.OUTPUT_IMG/'arma_garch'/f'{row}.png')
            plt.close()
